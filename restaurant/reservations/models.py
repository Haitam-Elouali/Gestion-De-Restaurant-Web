...
class Reservation(models.Model):
    ...
    def clean(self):
        ...
        # Convertir les chaînes en objets date/time (cas API avant save complet)
        if isinstance(self.date, str):
            try:
                self.date = _dt.strptime(self.date, '%Y-%m-%d').date()
            except (ValueError, TypeError):
                raise ValidationError("Format de date invalide (attendu YYYY-MM-DD).")
        if isinstance(self.heure, str):
            try:
                self.heure = _dt.strptime(self.heure[:5], '%H:%M').time()
            except (ValueError, TypeError):
                raise ValidationError("Format d'heure invalide (attendu HH:MM).")
